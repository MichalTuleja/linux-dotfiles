...
SCALE_STEP = 0.05          # round scale to nearest 0.05 (1.00, 1.05, 1.10, ...)
...
def ensure_wayland_env() -> dict:
    """
    Ensure XDG_RUNTIME_DIR, WAYLAND_DISPLAY and DBUS_SESSION_BUS_ADDRESS are present.
    - XDG_RUNTIME_DIR -> /run/user/<uid> if missing
    - WAYLAND_DISPLAY -> newest wayland-* socket in XDG_RUNTIME_DIR if missing
    - DBUS_SESSION_BUS_ADDRESS -> unix:path=$XDG_RUNTIME_DIR/bus if bus exists
    Returns a dict suitable for passing as env=...
    """
    env = os.environ.copy()
    # XDG_RUNTIME_DIR
    xr = env.get("XDG_RUNTIME_DIR")
    if not xr or not os.path.isdir(xr):
        xr = f"/run/user/{os.getuid()}"
        env["XDG_RUNTIME_DIR"] = xr
    # WAYLAND_DISPLAY
    wd = env.get("WAYLAND_DISPLAY")
    if not wd or not os.path.exists(os.path.join(xr, wd)):
        sockets = [p for p in glob.glob(os.path.join(xr, "wayland-*")) if os.path.exists(p)]
        if sockets:
            sockets.sort(key=os.path.getmtime, reverse=True)
            env["WAYLAND_DISPLAY"] = os.path.basename(sockets[0])
    # DBus session bus
    if not env.get("DBUS_SESSION_BUS_ADDRESS"):
        bus = os.path.join(xr, "bus")
...
def run_wlr_randr() -> str:
    ...
    try:
        env = ensure_wayland_env()
        cp = subprocess.run([WLR_RANDR_BIN_PATH], text=True, capture_output=True, env=env)
        if cp.returncode != 0:
            raise RuntimeError(cp.stderr.strip() or cp.stdout.strip() or f"exit {cp.returncode}")
        return cp.stdout
    except Exception as e:
        env = ensure_wayland_env()
        print(f"Error: failed to run wlr-randr: {e}", file=sys.stderr)
...
def pick_best_output(outputs: List[Dict]) -> Optional[Dict]:
    ...
    chosen_score, out, bm = candidates[0]
    out = out.copy()
    out["best_mode"] = bm
    return out
...
def build_wlr_randr_cmd(selected: Dict, all_outputs: List[Dict], scale: float) -> List[str]:
    ...
    w, h = bm["w"], bm["h"]
    hz = bm["hz_str"]
    args: List[str] = [WLR_RANDR_BIN_PATH, "--output", name, "--on", "--mode", f"{w}x{h}@{hz}", "--pos", "0,0", "--scale", f"{scale:.2f}"]
...
def notify(msg: str) -> bool:
    ...
    try:
        env = ensure_wayland_env()
        subprocess.run(["notify-send", msg], check=True, env=env)
        return True
...
def main():
    ...
    if "--apply" in sys.argv:
        print("\n# Applying…")
        try:
            subprocess.run(cmd, check=True, env=ensure_wayland_env())
            notify(message)
